...
geometry_msgs::Pose arm_to_base(geometry_msgs::Pose arm_pose){ 
    ...
    // Convert to RPY
    tf2::Quaternion q_orig(arm_pose.orientation.x,arm_pose.orientation.y,arm_pose.orientation.z,arm_pose.orientation.w);
    double roll, pitch, yaw;
    tf2::Matrix3x3(q_orig).getRPY(roll, pitch, yaw);
    // Remove rotations about x and y
    roll = 0.0;
    pitch = 0.0;
    // convert back to quaternions and overwrite
    tf2::Quaternion q_new;
    ...
    q_new.normalize();
    arm_pose.orientation.x = q_new.x();
    arm_pose.orientation.y = q_new.y();
    arm_pose.orientation.z = q_new.z();
    arm_pose.orientation.w = q_new.w();
    //// change frame from arm_1_link to torso_footprint  - would be more correct to use real transforms instead of measures like this
    arm_pose.position.x -= 0.09305;
    arm_pose.position.y -= 0.014;
    arm_pose.position.z -= 0.7375;
    //// the z value is how much we want the torso to move up - !! stay within joint ranges
    torso_value=arm_pose.position.z;
    if (torso_value < 0.0) {
        torso_value = 0.0;
    } else if (torso_value >= 0.35) {
        torso_value = 0.35;
    }
    ...
    if(service_avail){
        ...
        for(int i=0;i<poses.size();i++){
            ...
            if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED){
                ...
                res.success=true;
                std::string appo= "Reached pose "+std::to_string(i)+" ("+std::to_string(goal.target_pose.pose.position.x)+","+std::to_string(goal.target_pose.pose.position.y)+","+std::to_string(goal.target_pose.pose.position.z)+") ("+std::to_string(goal.target_pose.pose.orientation.x)+","+std::to_string(goal.target_pose.pose.orientation.y)+","+std::to_string(goal.target_pose.pose.orientation.z)+","+std::to_string(goal.target_pose.pose.orientation.w)+")";
                res.message= appo;
                if (move_torso()){
