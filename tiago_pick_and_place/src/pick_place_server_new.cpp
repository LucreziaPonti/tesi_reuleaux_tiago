...
public:
    PickPlaceManager(ros::NodeHandle& nh) : nh_(nh), move_group("arm") {

        // Creazione client necessario
        gazebo_model_cl= nh_.serviceClient<gazebo_msgs::GetModelState>("/gazebo/get_model_state");
        ...
        move_all_srv= nh_.advertiseService("move_obj_on_tiago", &PickPlaceManager::move_all_cb, this);
        pick_name_srv= nh_.advertiseService("pick_object", &PickPlaceManager::pick_name_cb, this);
        place_name_srv= nh_.advertiseService("place_object", &PickPlaceManager::place_name_cb, this);
    }
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "pick_and_place_server_new");
    ros::NodeHandle nh;
    
    PickPlaceManager manager(nh);
